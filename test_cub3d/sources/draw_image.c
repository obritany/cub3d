...
void	hit_horizontal(t_game *gm)
{
	...
	{
		if (gm->map.array[(int)(y - 1 + gm->ray.s)][(int)floor(x)] == ' ')
			break ;
		if (gm->map.array[(int)(y - 1 + gm->ray.s)][(int)floor(x)] == '1')
		{
...
void	hit_vertical(t_game *gm)
{
	...
	{
		if (gm->map.array[(int)floor(y)][(int)(x - 1 + gm->ray.e)] == ' ')
			break ;
		if (gm->map.array[(int)floor(y)][(int)(x - 1 + gm->ray.e)] == '1')
		{
			gm->hit.x = x;
			gm->hit.y = y;
			gm->hit.d = hypot(x - gm->pos.x, y - gm->pos.y);
...
void	get_ray_dir(t_game *gm)
{
	...
	ang = (double)gm->ray.i / (gm->win.x / 2) - 1;		//ang from -1 to 1
...
int		draw_image(t_game *gm)
{
	...
	gm->img.adr = (unsigned int *)mlx_get_data_addr(gm->img.ptr, &bits, &size, &endian);
